...
class Configuration {
  ...
  Set<String> get knownTags {
    ...
    var known = suite.knownTags.toSet();
  ...
  final SuiteConfiguration suite;
  ...
  Configuration._({
      ...
      : _help = help,
        ...
        suite = (pauseAfterLoad ?? false)
            ? suite?.change(timeout: Timeout.none) ??
                  new SuiteConfiguration(timeout: Timeout.none)
            : suite ?? SuiteConfiguration.empty {
  ...
  Configuration _resolvePresets() {
    ...
    var newPresets = new Map.from(presets);
    ...
    if (result == Configuration.empty) return this;
